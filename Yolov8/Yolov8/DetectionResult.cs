...
namespace ResultSharp
{
    public class DetectionResult: ResultBase
    {
        ...
        {
            Mat result_data = new Mat(5, 8400, MatType.CV_32F, result);
            ...
            {
                Mat classes_scores = result_data.Row(i).ColRange(4, 5);//GetArray(i, 5, classes_scores);
        ...
        public Mat draw_result(Result result, Mat image) {
            ...
            Cv2.Rectangle(image, result.rects[0], new Scalar(0, 0, 255), 2, LineTypes.Link8);
            Mat croppedImage = new Mat(image, result.rects[0]);

            string outputPath = "cropped_image.jpg";  // 替换为你想要保存的路径和文件名
           ...
           /* for (int i = 0; i < result.length; i++)
            {
                ...
                Mat croppedImage = new Mat(image, result.rects[i]);
